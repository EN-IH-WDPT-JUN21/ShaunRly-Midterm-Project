        ...
        if (storedChecking.isPresent()){
            ...
            if(checking.getMinimumBalance() != null){
                try {
                    ...
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "MoMinimum Balance formatted incorrectly.");
                }
            }
            System.out.println(storedSuperUpdated);
            checkingRepository.save(storedSuperUpdated);
        }
        else{
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "The Checking Account does not exists");
        }
